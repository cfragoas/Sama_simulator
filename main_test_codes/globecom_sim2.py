...
from macel_globecom import Macel
...
def simulate_ue_macel(args):
    ...
    macel = args[1]
    tot_samples = args[2]
    samples = np.round(tot_samples/n_bs).astype(int)

    macel.grid.make_points(dist_type='gaussian', samples=samples, n_centers=n_bs, random_centers=False,
                          plot=False)  # distributing points around centers in the grid
    predetermined_centroids = macel.grid.centers_set
    macel.set_ue(hrx=1.5)
    snr_cap_stats, raw_data = macel.place_and_configure_bs(
        n_centers=n_bs, output_typ='complete', clustering=False, predetermined_centroids=predetermined_centroids)
...
if __name__ == '__main__':
    # parameters
    criteria = 30  # Mbps
    tot_samples = 1000  # number total of samples to be splitted betweem all gaussians
    max_iter = 1000
    ...
    macel = Macel(grid=grid, prop_model='free space', criteria=criteria, cell_size=cell_size, base_station=base_station, simulation_time=simulation_time, scheduling_opt=scheduling_opt)
    ...
    for n_cells in bs_range:
        ...
        data = list(
                    tqdm.tqdm(p.imap_unordered(simulate_ue_macel, [(n_cells, macel, tot_samples) for i in range(max_iter)]), total=max_iter
                ))
        snr_cap_stats = [x[0] for x in data]
        raw_data = [x[1] for x in data]
